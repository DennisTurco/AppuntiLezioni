class Speculatore:
    ...
    def maxProfitVersion1(self, azioni, prezzi):
        # caso errore
        if len(prezzi) <= 0:
            raise Exception(f"il numero dello storico dei prezzi non puo' essere {len(prezzi)} (<= 0)")
    ...
    def maxProfitVersion4(self, k, P):
        # caso errore
        if len(P) <= 0:
            raise Exception(f"il numero dello storico dei prezzi non puo' essere {len(P)} (<= 0)")
